...
public class Base58
{
    ...
    public static string Base58String
    {
        ...
        set
        {
            ...
            {
                if (!_base58Charset.Contains(ch))
                {
                    throw new ArgumentException($"Character {ch} is not a valid base58 symbol!");
                }
            });
            _base58String = value;
    ...
    public static byte[] ByteArray
    {
        ...
        set
        {
            _byteArray = value ?? throw new ArgumentNullException();
            _BE_number = new BigInteger(_byteArray);
            _LE_number = new BigInteger(_byteArray.Reverse().ToArray());
            _base58String = string.Empty;
            Encode(_BE_number);
    ...
    private static void Decode() => _BE_number = Base58String
        ...
        .Aggregate(BigInteger.Zero, (agg, base58tuple) =>
            agg.Add(new BigInteger(_base58Charset.IndexOf(base58tuple.Item1).ToString()).Multiply(
                new BigInteger("58").Pow(base58tuple.Item2))));

    private static void Base58ZeroPad()
    {
        foreach (var by in _byteArray)
        {
    ...
    {
        foreach (var sy in _base58String)
        {
